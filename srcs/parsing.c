...
/*   Updated: 2022/09/20 14:06:49 by skhali           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_args(int argc, char **argv)
{
	...
	if (argc == 6)
		if (!is_integer(argv[5]) || (atol(argv[5]) < 0))
			return (printf("Wrong n/t of time each philo must eat.\n"), 0);
	return (1);
}
...
int	parsing(int argc, char **argv, t_stat *stat)
{
	...
	if (!stat->philos)
		return (free(stat->philos), free(stat),
			printf("Malloc error on forks.\n"), 0);
	return (1);
}
